...
export const toolOrder = ['position', 'shields', 'custom', 'charges', 'top', 'colors', 'tools', 'how-to', 'settings', 'flags', 'tokens'] as const;
...
export interface CoatWorkbenchCopy {
  ...
  localProjectsDescription: string;
  close: string;
  projectName: string;
  newProject: string;
  saveProject: string;
  ...
  projectSaved: string;
  projectCopied: string;
  projectRenamed: string;
  projectDeleted: string;
  projectLoaded: string;
  newProjectReady: string;
  projectDownloaded: string;
  projectImported: string;
  projectImportFileTooLarge: (fileSize: number, maxFileSize: number) => string;
  projectNameRequired: string;
  selectProjectToRename: string;
  selectProjectToDelete: string;
  selectProjectToLoad: string;
  projectUnavailable: (projectId: string) => string;
  defaultMotto: string;
  projectOperationFailed: (message: string) => string;
  ...
  utilityTabs: Record<'text' | 'draw' | 'names' | 'upload' | 'layers', string>;
  shell: {
    ...
    openLocalProjectLibrary: string;
    multiSelect: string;
    canvasZoom: string;
  ...
  canvas: {
    ...
    cropSelectedLayer: string;
    moveCropFrame: string;
    resizeCropLeftEdge: string;
    resizeCropRightEdge: string;
    resizeCropTopEdge: string;
    resizeCropBottomEdge: string;
  };
  panels: {
    editorTools: string;
    ...
    generatedMotto: string;
    generateIdentity: string;
    useProjectName: string;
    addGeneratedMotto: string;
...
const workbenchCopyByLocale: Record<CoatLocale, CoatWorkbenchCopy> = {
  en: {
    ...
    localFirstMaker: 'Local-first maker', undo: 'Undo', redo: 'Redo', projects: 'Projects', export: 'Export', scene: 'Coat scene',
    ...
    localProjects: 'Local projects', localProjectsDescription: 'Saved only in this browser.', close: 'Close', projectName: 'Project name',
    newProject: 'New project', saveProject: 'Save project locally', saveAsProject: 'Save as local copy', exportProjectJson: 'Export project JSON',
    importProjectJson: 'Import project JSON', importProjectJsonFile: 'Import project JSON file', savedProjects: 'Saved projects',
    noSelectedProject: 'No saved project selected', noProjects: 'No local projects yet.', loadProject: 'Load saved project',
    renameProject: 'Rename saved project', deleteProject: 'Delete saved project', projectSaved: 'Project saved locally.',
    projectCopied: 'Local copy created.', projectRenamed: 'Saved project renamed.', projectDeleted: 'Saved project deleted.',
    projectLoaded: 'Saved project loaded.', newProjectReady: 'New local project ready.', projectDownloaded: 'Project JSON downloaded.',
    projectImported: 'Project JSON imported locally.', projectImportFileTooLarge: (fileSize, maxFileSize) => `Project JSON file is too large (${fileSize} bytes; maximum ${maxFileSize} bytes).`, projectNameRequired: 'Project name cannot be empty',
    selectProjectToRename: 'Choose a saved project to rename', selectProjectToDelete: 'Choose a saved project to delete',
    selectProjectToLoad: 'Choose a saved project to load', projectUnavailable: (projectId) => `Saved project is unavailable: ${projectId}`,
    defaultMotto: 'FORTUNE FAVOURS', projectOperationFailed: (message) => `Project operation failed: ${message}`, exportOptions: 'Local export options',
    closeExportMenu: 'Close export menu', exportSize: 'Export size', exportPng: 'Export PNG', exportJpeg: 'Export JPG', copyImage: 'Copy image', shareImage: 'Share image', jpegQuality: 'JPG quality', jpegQualities: { low: 'Low', medium: 'Medium', high: 'High', ultra: 'Ultra' }, exportPdf: 'Export PDF',
    ...
    randomizeProject: 'Randomize project', randomizeUnavailable: 'Project randomization is not available yet.',
    ...
    toolTabs: { position: 'Position', shields: 'Shields', custom: 'Custom', charges: 'Charges', top: 'Top', colors: 'Colors', tools: 'Tools', 'how-to': 'How-to', settings: 'Settings', flags: 'Flags', tokens: 'Tokens' },
    utilityTabs: { text: 'Text', draw: 'Draw', names: 'Names', upload: 'Upload', layers: 'Layers' },
    shell: {
    ...
    panels: {
      ...
      settings: 'Settings', canvasSize: 'Canvas size', canvasWidth: 'Canvas width', canvasHeight: 'Canvas height', applyCanvasSize: 'Apply canvas size', resetEditor: 'Reset editor', canvasPresets: 'Canvas presets', canvasPresetNames: { square: 'Square', 'instagram-story': 'Instagram Story', portrait: 'Portrait', landscape: 'Landscape' },
      draw: 'Draw', drawingColour: 'Drawing colour', drawingStrokeWidth: 'Drawing stroke width', drawOnCanvas: 'Draw on canvas', drawHelp: 'Enable freehand drawing, then drag across the canvas to add a local vector stroke.',
      names: 'Names', generatedProjectName: 'Generated project name', generatedMotto: 'Generated motto', generateIdentity: 'Generate identity', useProjectName: 'Use project name', addGeneratedMotto: 'Add generated motto',
      coloursAndBackground: 'Colours & background', usedColours: 'Used colours', customPaletteColour: 'Custom palette colour', saveCustomColour: 'Save custom colour', replaceColourFrom: 'Replace colour from', replaceColourWith: 'Replace colour with', replaceAllColours: 'Replace all colours', backgroundColour: 'Background colour', customBackgroundColour: 'Custom background colour', backgroundGradient: 'Background gradient', backgroundGradientAngle: 'Background gradient angle', backgroundGradientStartColour: 'Background gradient start colour', backgroundGradientEndColour: 'Background gradient end colour', applyBackgroundGradient: 'Apply background gradient', backgroundMotif: 'Background motif', backgroundOpacity: 'Background opacity', transparentExportBackground: 'Transparent export background', backgroundVisible: 'Background visible', perLayerColours: 'Per-layer colours', colourForLayer: (layerId) => `Colour for ${layerId}`, colourForAssetPart: (layerId, sourceColor) => `Colour for ${layerId} part ${sourceColor}`,
      textAndMotto: 'Text & motto', mottoText: 'Motto text', textColour: 'Text colour', typographySize: 'Typography size', fontFamily: 'Font family', textFonts: { serif: 'Serif', 'display-serif': 'Display serif', blackletter: 'Blackletter', 'sans-serif': 'Sans serif', monospace: 'Monospace', cursive: 'Cursive' }, fontStyle: 'Font style', fontWeight: 'Font weight', textFontStyles: { normal: 'Normal', italic: 'Italic' }, textFontWeights: { normal: 'Normal', bold: 'Bold' }, textAlignment: 'Text alignment', textPath: 'Text path', textTransform: 'Text transform', x: 'X', y: 'Y', scale: 'Scale', rotation: 'Rotation', textAlignments: { left: 'Left', center: 'Center', right: 'Right' }, textPaths: { none: 'None', 'motto-upper': 'Motto upper', 'motto-lower': 'Motto lower', 'curve-upper': 'Curve upper', 'curve-lower': 'Curve lower', 'ring-clockwise': 'Ring clockwise', 'ring-counterclockwise': 'Ring counterclockwise' }, addMotto: 'Add motto', updateSelectedText: 'Update selected text',
      uploadImage: 'Upload image', uploadDescription: 'PNG, JPEG, WebP, or safe SVG; local files only, up to 8 files, maximum 256 KB each and 512 KB total.', uploadCrestImage: 'Upload crest image', localUploads: 'Local uploads', localUploadItem: (index, mimeType) => `Local upload ${index + 1} (${mimeType})`, addLocalImage: (index) => `Add local image ${index + 1}`, removeLocalUpload: (index) => `Remove local upload ${index + 1}`, localImageAdded: (fileName) => `Added local image: ${fileName}`, localImagesAdded: (count) => `Added ${count} local images.`,
      layers: 'Layers', coatLayers: 'Coat layers', selectLayer: (layerId, name) => `Select layer ${layerId} (${name})`, select: 'Select', layerType: (type) => ({ background: 'background', shield: 'shield', ordinary: 'ordinary', charge: 'charge', top: 'top ornament', draw: 'drawing', text: 'text', image: 'image' })[type] ?? type, moveLayerUp: (name) => `Move ${name} up`, moveLayerDown: (name) => `Move ${name} down`, hideLayer: (name) => `Hide ${name}`, showLayer: (name) => `Show ${name}`, lockLayer: (name) => `Lock ${name}`, unlockLayer: (name) => `Unlock ${name}`, deleteLayer: (name) => `Delete ${name}`, moveUp: 'Move up', moveDown: 'Move down', hide: 'Hide', show: 'Show', lock: 'Lock', unlock: 'Unlock', delete: 'Delete', duplicateSelectedLayers: 'Duplicate selected layers', groupSelectedLayers: 'Group selected layers', ungroupSelectedLayers: 'Ungroup selected layers', groupOpacity: 'Group opacity', localImage: 'Local image', localDrawing: 'Drawing',
    },
  },
  zh: {
    workspace: '徽章制作工作台', desktopTools: '桌面徽章工具', toolCategories: '徽章工具类别', toolRailLabel: '徽章制作工具', layerDock: '项目与图层停靠区',
    localFirstMaker: '本地优先制作器', undo: '撤销', redo: '重做', projects: '项目', export: '导出', scene: '徽章画布',
    projectAndLayers: '项目与图层', layerDockHint: '选择图层以整理、锁定或分组。', inspector: '属性', tools: '工具', closeTools: '关闭工具',
    localProjects: '本地项目', localProjectsDescription: '仅保存在当前浏览器中。', close: '关闭', projectName: '项目名称',
    newProject: '新建项目', saveProject: '本地保存项目', saveAsProject: '另存为本地副本', exportProjectJson: '导出项目 JSON',
    importProjectJson: '导入项目 JSON', importProjectJsonFile: '导入项目 JSON 文件', savedProjects: '已保存项目',
    noSelectedProject: '尚未选择已保存项目', noProjects: '还没有本地项目。', loadProject: '加载已保存项目',
    renameProject: '重命名已保存项目', deleteProject: '删除已保存项目', projectSaved: '项目已保存到本地。',
    projectCopied: '已创建本地副本。', projectRenamed: '已重命名保存的项目。', projectDeleted: '已删除保存的项目。',
    projectLoaded: '已加载保存的项目。', newProjectReady: '新的本地项目已准备就绪。', projectDownloaded: '项目 JSON 已下载。',
    projectImported: '项目 JSON 已导入本地。', projectImportFileTooLarge: (fileSize, maxFileSize) => `项目 JSON 文件过大（${fileSize} 字节，最大 ${maxFileSize} 字节）。`, projectNameRequired: '项目名称不能为空',
    selectProjectToRename: '请选择要重命名的已保存项目', selectProjectToDelete: '请选择要删除的已保存项目',
    selectProjectToLoad: '请选择要加载的已保存项目', projectUnavailable: (projectId) => `无法找到已保存项目：${projectId}`,
    defaultMotto: '勇气与荣耀', projectOperationFailed: (message) => formatChineseFailure('项目操作失败', '请检查项目数据后重试', message), exportOptions: '本地导出选项',
    closeExportMenu: '关闭导出菜单', exportSize: '导出尺寸', exportPng: '导出 PNG', exportJpeg: '导出 JPG', copyImage: '复制图片', shareImage: '分享图片', jpegQuality: 'JPG 质量', jpegQualities: { low: '低', medium: '中', high: '高', ultra: '超高' }, exportPdf: '导出 PDF',
    ...
    randomizeProject: '随机生成项目', randomizeUnavailable: '项目随机生成功能暂不可用。',
    ...
    toolTabs: { position: '定位', shields: '盾牌', custom: '自定义', charges: '图形', top: '顶部', colors: '颜色', tools: '工具', 'how-to': '使用说明', settings: '设置', flags: '旗帜', tokens: '符记' },
    utilityTabs: { text: '文字', draw: '绘制', names: '命名', upload: '上传', layers: '图层' },
    shell: {
      brand: { firstLine: 'TOKEN', secondLine: 'MAKER', accent: 'STUDIO' },
    ...
    panels: {
      ...
      fieldPatterns: { solid: '纯色', stripes: '条纹', dots: '圆点', checks: '棋盘格', lozengy: '菱格', crosses: '十字', waves: '波浪', masoned: '砌石纹', honeycomb: '蜂巢纹', fretty: '交织纹', scales: '鳞片纹', chevronelly: '连续人字纹', vair: '松鼠皮纹', 'vair-in-pointe': '尖角松鼠皮纹', 'vair-in-pale': '纵向松鼠皮纹', 'paly-bendy': '竖斜复合条纹', 'barry-bendy': '横斜复合条纹', gyronny: '放射纹', papelonny: '扇贝纹', seme: '散点纹' },
      ordinariesAndCharges: '饰带与徽记', libraryCategory: '素材类别', charges: '徽记', chargeCategory: '徽记类别', chargeCategories: { animal: '动物', object: '物件', plant: '植物', human: '人物', symbol: '符号' }, ordinaries: '饰带', searchLibrary: '搜索素材', addRandom: (kind) => `随机添加${kind}`, noLocalAssets: (kind) => `没有可用的本地${kind}素材`, assetLibrary: (kind) => `${kind}素材库`, addAsset: (name) => `添加${name}`, previewAsset: (name) => `预览素材：${name}`,
      top: '顶部装饰', topCategory: '顶部类别', topLibrary: '顶部装饰素材库', topCategories: { mantle: '斗篷', crown: '冠冕', supporter: '护持者', other: '其他' },
      settings: '设置', canvasSize: '画布尺寸', canvasWidth: '画布宽度', canvasHeight: '画布高度', applyCanvasSize: '应用画布尺寸', resetEditor: '重置编辑器', canvasPresets: '画布预设', canvasPresetNames: { square: '方形', 'instagram-story': 'Instagram 快拍', portrait: '竖版', landscape: '横版' },
      draw: '绘制', drawingColour: '绘制颜色', drawingStrokeWidth: '笔触宽度', drawOnCanvas: '在画布上绘制', drawHelp: '开启自由绘制后，在画布上拖动即可添加本地矢量笔迹。',
      names: '命名', generatedProjectName: '生成的项目名称', generatedMotto: '生成的格言', generateIdentity: '生成名称与格言', useProjectName: '使用项目名称', addGeneratedMotto: '添加生成的格言',
      coloursAndBackground: '颜色与背景', usedColours: '已用颜色', customPaletteColour: '自定义调色板颜色', saveCustomColour: '保存自定义颜色', replaceColourFrom: '替换来源颜色', replaceColourWith: '替换为颜色', replaceAllColours: '替换全部颜色', backgroundColour: '背景颜色', customBackgroundColour: '自定义背景颜色', backgroundGradient: '背景渐变', backgroundGradientAngle: '背景渐变角度', backgroundGradientStartColour: '背景渐变起始颜色', backgroundGradientEndColour: '背景渐变结束颜色', applyBackgroundGradient: '应用背景渐变', backgroundMotif: '背景纹样', backgroundOpacity: '背景不透明度', transparentExportBackground: '透明导出背景', backgroundVisible: '显示背景', perLayerColours: '各图层颜色', colourForLayer: (layerId) => `图层 ${layerId} 的颜色`, colourForAssetPart: (layerId, sourceColor) => `图层 ${layerId} 色块 ${sourceColor}`,
      textAndMotto: '文字与格言', mottoText: '格言文字', textColour: '文字颜色', typographySize: '字号', fontFamily: '字体', textFonts: { serif: '衬线体', 'display-serif': '展示衬线体', blackletter: '哥特黑体', 'sans-serif': '无衬线体', monospace: '等宽体', cursive: '手写体' }, fontStyle: '字体样式', fontWeight: '字体字重', textFontStyles: { normal: '常规', italic: '斜体' }, textFontWeights: { normal: '常规', bold: '粗体' }, textAlignment: '文字对齐', textPath: '文字路径', textTransform: '文字变换', x: '横向位置', y: '纵向位置', scale: '缩放', rotation: '旋转', textAlignments: { left: '左对齐', center: '居中', right: '右对齐' }, textPaths: { none: '无', 'motto-upper': '格言上弧', 'motto-lower': '格言下弧', 'curve-upper': '上弧', 'curve-lower': '下弧', 'ring-clockwise': '顺时针环绕', 'ring-counterclockwise': '逆时针环绕' }, addMotto: '添加格言', updateSelectedText: '更新选中文字',
      uploadImage: '上传图片', uploadDescription: '支持 PNG、JPEG、WebP 或安全 SVG；仅使用本地文件，最多 8 个文件，单个最大 256 KB，总计最大 512 KB。', uploadCrestImage: '上传徽章图片', localUploads: '本地上传素材', localUploadItem: (index, mimeType) => `本地上传素材 ${index + 1}（${mimeType}）`, addLocalImage: (index) => `添加本地图片 ${index + 1}`, removeLocalUpload: (index) => `移除本地上传素材 ${index + 1}`, localImageAdded: (fileName) => `已添加本地图片：${fileName}`, localImagesAdded: (count) => `已添加 ${count} 个本地图片。`,
      layers: '图层', coatLayers: '徽章图层', selectLayer: (layerId, name) => `选择图层 ${layerId}（${name}）`, select: '选择', layerType: (type) => ({ background: '背景', shield: '盾牌', ordinary: '饰带', charge: '徽记', top: '顶部装饰', draw: '绘制笔迹', text: '文字', image: '图片' })[type] ?? type, moveLayerUp: (name) => `将 ${name} 上移`, moveLayerDown: (name) => `将 ${name} 下移`, hideLayer: (name) => `隐藏 ${name}`, showLayer: (name) => `显示 ${name}`, lockLayer: (name) => `锁定 ${name}`, unlockLayer: (name) => `解锁 ${name}`, deleteLayer: (name) => `删除 ${name}`, moveUp: '上移', moveDown: '下移', hide: '隐藏', show: '显示', lock: '锁定', unlock: '解锁', delete: '删除', duplicateSelectedLayers: '复制选中图层', groupSelectedLayers: '将选中图层成组', ungroupSelectedLayers: '取消选中图层成组', groupOpacity: '组不透明度', localImage: '本地图片', localDrawing: '绘制笔迹',
    },
  },
};
